...
def analyze_input_filtergraph_ids(
    fg: FilterGraphObject | str,
) -> list[tuple[int, MediaType]]:
    """get labels and media types of input filtergraph output pads

    :param fg: input filtergraph expression/object
    :return: list of indices and types of input streams

    FFmpeg Input Filtergraph Output Specification:
    ----------------------------------------------

    Each video open output must be labelled by a unique string of the form `"outN"`,
    where `N` is a number starting from `0` corresponding to the mapped input stream
    generated by the device. The first unlabelled output is automatically assigned
    to the `"out0"` label, but all the others need to be specified explicitly.

    The suffix "+subcc" can be appended to the output label.
    ...
    for idx, filter, _ in fg.iter_output_pads():
        ...
        if label is None:
            if 0 in outtypes:
                raise FFmpegError(
                    "more than one `out0` pad specified in the input filtergraph."
                )
            outid = 0
        else:
            m = re.match(r"out(\d+)(?:\+subcc)?$", label)
            if not m:
                raise FFmpegError(
                    f"Specified input filtergraph contains an invalid output pad label: {label}"
                )
            outid = int(m[1])
            if outid in outtypes:
                raise FFmpegError(
                    f"Specified input filtergraph defines more than one `out{outid}` pad label."
                )

        outtypes[outid] = filter.get_pad_media_type("output", idx[-1])

    return list((i, outtypes[i]) for i in sorted(outtypes))
