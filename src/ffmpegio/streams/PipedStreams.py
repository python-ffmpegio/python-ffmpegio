...
from .. import configure, plugins, utils
...
class _RawInputsMixin(_BaseRawInputsMixin):
...
class _RawOutputsMixin(_BaseRawOutputsMixin):
    ...
    def read(
        self, n: int, stream_id: int | str = 0, timeout: float | None = None
    ) -> RawDataBlob:
    ...
    def readall(self, n: int, timeout: float | None = None) -> dict[str, RawDataBlob]:
...
class MediaTranscoder(_EncodedOutputsMixin, _EncodedInputsMixin, _PipedFFmpegRunner):
    ...
    def __init__(
        ...
        **options: Unpack[FFmpegOptionDict],
        ...
        :param ref_output: index or label of the reference stream to pace read operation, defaults to 0.
                           `MediaFilter.read()` operates around the reference stream.
