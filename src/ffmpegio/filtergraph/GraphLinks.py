...
class GraphLinks(UserDict):
    ...
    @staticmethod
    def validate_label(label: str | int, is_link: bool = True) -> str | int:
        """validate a string for filtergraph label

        NOTE: Validation will fail for input stream mapping labels like 'v:0'
            although it is a valid label

        :param label: label string to be validated. The square brackets
            (`'[label]'`) is optional. Label may be a non-negative integer if it
            is only used to link filter pads.
        :param is_link: False to invalidate an integer label, defaults to True
        :return: validated label without square brackets
        ...
        if isinstance(label, int):
            ...
            if label < 0:
                raise GraphLinks.Error("An integer pad label must be non-negative.")
    ...
    def link(
        ...
        if inlabel is True:
            ...
            # force=True -> unlink existing link first
    ...
    def create_label(
        ...
        if not force and isinstance(inpad_, tuple) and label_ in self:
    ...
    def _is_inpad_used(self, pad: PAD_INDEX_) -> bool | str:
        ...
        :return: ``True`` if ``pad`` is already connected to an input pad, or
            the input label ``str`` if the pad has a label but not connected
    ...
    def combine(
        ...
        for i, (obj, cid0) in enumerate(zip(link_objs, cumsum_chains)):
            ...
            for label in links:
                new_label = label  # new label, use existing if not duplicate
                if isinstance(new_label, int):
                    new_label = int_counter
                    int_counter += 1
                    mappings[i][label] = new_label
                elif links.is_input_stream(label):
                    # update the connected input pads
                    input_streams[label].extend(
                        [(cid + cid0, fid, pid) for cid, fid, pid in links[label][0]]
                    )
                else:  # string labels
                    assert isinstance(new_label, str)
                    # check if label has a trailing index
                    m = regexp.search(new_label)
                    key = m[1] if m else new_label

                    if label in unique_labels:
                        # the pad label already used, set the trailing index to minimum unused
                        next_label_id[key] = i = next_label_id[key] + 1
                        # create new label by append trailing index
                        new_label = f"{key}{i}"
                    else:
                        # new pad label, keep the maximum trailing digit tracker
                        j = int(m[2]) if m else 0
                        if next_label_id.get(key, -1) < j:
                            next_label_id[key] = j
    ...
    def combine_chains(
        self, out_pad: PAD_INDEX, in_pad: PAD_INDEX, n_input: int, prepend: bool = False
    ):
        """adjust pad indices as two chains are combined

        :param cid_out: id of the connecting output pad of the input chain
        :param cid_in: id of the connecting input pad of the output chain
        :param n_input: number of filters of the input chain
        :param prepend: True to move input chain to output chain,
            defaults to False, moving output chain to input chain
        ...
        cid_out, cid_in = out_pad[0], in_pad[0]  # input/output chain ids
        cid_host, cid_move = (cid_in, cid_out) if prepend else (cid_out, cid_in)

        # update all the pad indices appearing after the moving chain

        # ADJUSTMENT RULES:
        # - the indices of all chains after cid_mov must be reduced by 1
        # - if prepend=False, the indices of all filters on the moving chain must be adjusted
        # - if prepend=True, the indices of all filters on the host chain must be adjusted

        def adjust_padidx(pad: PAD_INDEX_) -> PAD_INDEX_:
            cid, fid, pid = pad
            if (prepend and cid == cid_host) or (not prepend and cid == cid_move):
                fid += n_input
            if cid == cid_move:
                cid = cid_host
            if cid > cid_move:
        ...
        new_items = {
            label: (
                ...
                and (
                    ...
                    else [adjust_padidx(pad) for pad in inpad]
                ),
                outpad and adjust_padidx(outpad),
            )
            for label, (inpad, outpad) in self.items()
        }
    ...
    def are_linked(
        ...
        if inpad is None:  # any link with outpad
            return any(
                inp is not None and outp == outpad
                for _, inp, outp in self.iter_output_pads()
            )
