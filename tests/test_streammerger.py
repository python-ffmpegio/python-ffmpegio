...
from ffmpegio.streams.StreamMerger import EncodedStreamMerger
...
def test_merger():
    ...
    with open(video_url, "rb") as vf, open(audio_url, "rb") as af, EncodedStreamMerger(
        nb_inputs=2, show_log=True
    ) as merger:
        while True:
            vdata = vf.read(vsz)
            if vdata:
